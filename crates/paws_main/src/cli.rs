...
//! For example: `paws provider login` instead of `paws providers login`.
//!
//! NOTE: With every change to this CLI structure, verify that the ZSH plugin
//! remains compatible. The plugin at `shell-plugin/paws.plugin.zsh` implements
...
impl From<Scope> for paws_domain::Scope {
    fn from(value: Scope) -> Self {
        match value {
            Scope::Local => paws_domain::Scope::Local,
            Scope::User => paws_domain::Scope::User,
        }
    }
}
...
impl From<DataCommandGroup> for paws_domain::DataGenerationParameters {
...
mod tests {
    ...
    fn test_data_command_group_conversion() {
        ...
        let actual: paws_domain::DataGenerationParameters = fixture.into();
        let expected = paws_domain::DataGenerationParameters {
    ...
    fn test_config_set_with_provider() {
        let fixture = Cli::parse_from(["paws", "config", "set", "provider", "OpenAI"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Config(config)) => match config.command {
    ...
    #[test]
    fn test_config_list() {
        let fixture = Cli::parse_from(["paws", "config", "list"]);
        let actual = match fixture.subcommands {
    ...
    fn test_config_get_specific_field() {
        let fixture = Cli::parse_from(["paws", "config", "get", "model"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Config(config)) => match config.command {
    ...
    fn test_conversation_list() {
        let fixture = Cli::parse_from(["paws", "conversation", "list"]);
    ...
    fn test_session_alias_list() {
        let fixture = Cli::parse_from(["paws", "session", "list"]);
        let is_list = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => {
                matches!(conversation.command, ConversationCommand::List { .. })
            }
    ...
    fn test_agent_id_long_flag() {
        let fixture = Cli::parse_from(["paws", "--agent", "sage"]);
        assert_eq!(fixture.agent, Some(AgentId::new("sage")));
    }

    #[test]
    fn test_agent_id_short_alias() {
        let fixture = Cli::parse_from(["paws", "--aid", "muse"]);
        assert_eq!(fixture.agent, Some(AgentId::new("muse")));
    }

    #[test]
    fn test_agent_id_with_prompt() {
        let fixture = Cli::parse_from(["paws", "--agent", "paws", "-p", "test prompt"]);
        assert_eq!(fixture.agent, Some(AgentId::new("paws")));
    ...
    fn test_list_tools_command_with_agent() {
        let fixture = Cli::parse_from(["paws", "list", "tool", "sage"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_conversation_command() {
        let fixture = Cli::parse_from(["paws", "list", "conversation"]);
    ...
    fn test_list_session_alias_command() {
        let fixture = Cli::parse_from(["paws", "list", "session"]);
        let is_conversation_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => matches!(list.command, ListCommand::Conversation),
            _ => false,
        };
    ...
    fn test_info_command_without_porcelain() {
        let fixture = Cli::parse_from(["paws", "info"]);
        let actual = match fixture.subcommands {
    ...
    fn test_info_command_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "info", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Info { porcelain, .. }) => porcelain,
            _ => false,
        };
    ...
    fn test_list_agents_without_porcelain() {
        let fixture = Cli::parse_from(["paws", "list", "agents"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_agents_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "list", "agents", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_mcp_list_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "mcp", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_conversation_list_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "conversation", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => match conversation.command {
    ...
    fn test_list_models_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "list", "models", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_config_list_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "config", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_cmd_command_with_args() {
        let fixture = Cli::parse_from(["paws", "cmd", "custom-command", "arg1", "arg2"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_skill_command() {
        let fixture = Cli::parse_from(["paws", "list", "skill"]);
    ...
    fn test_list_skills_alias_command() {
        let fixture = Cli::parse_from(["paws", "list", "skills"]);
        let is_skill_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => matches!(list.command, ListCommand::Skill),
            _ => false,
        };
    ...
    fn test_list_skill_with_porcelain() {
        let fixture = Cli::parse_from(["paws", "list", "skill", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
