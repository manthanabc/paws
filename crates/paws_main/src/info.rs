...
impl From<&Environment> for Info {
    fn from(env: &Environment) -> Self {
        ...
        info = info
            ...
            .add_key_value("Paws API URL", env.paws_api_url.to_string())
...
/// Create an info instance for available commands from a PawsCommandManager
impl From<&PawsCommandManager> for Info {
    fn from(command_manager: &PawsCommandManager) -> Self {
        ...
        info = info
            ...
            .add_key_value("<CTRL+D>", "Quit Paws interactive shell")
    ...
    fn from(user_usage: &UserUsage) -> Self {
        ...
        if plan.is_upgradeable() {
            info = info.add_key_value(
                "Subscription",
                format!(
                    "{} [Upgrade https://app.pawscode.dev/app/billing]",
...
mod tests {
    ...
    use paws_api::{Environment, EventValue};
    ...
    fn test_metrics_info_display() {
        use paws_api::Metrics;
        use paws_domain::{FileOperation, ToolKind};
    ...
    fn test_conversation_info_display() {
        ...
        let fixture = Conversation {
            ...
            context: None,
            metrics,
            metadata: paws_domain::MetaData::new(Utc::now()),
        };
    ...
    fn test_conversation_info_display_with_task() {
        use chrono::Utc;
        use paws_api::{Context, ContextMessage, ConversationId, Role};
        ...
        let context = Context::default()
            ...
            .add_message(ContextMessage::Text(
                paws_domain::TextMessage::new(Role::User, "First user message")
                    .raw_content(EventValue::text("First user message")),
            ))
            .add_message(ContextMessage::assistant("Assistant response", None, None))
            .add_message(ContextMessage::Text(
                paws_domain::TextMessage::new(Role::User, "Create a new feature")
    ...
    fn test_info_from_command_manager() {
        let command_manager = super::PawsCommandManager::default();
