...
pub struct PawsAPI<S, F> {
    services: Arc<S>,
    infra: Arc<F>,
}

impl<A, F> PawsAPI<A, F> {
    pub fn new(services: Arc<A>, infra: Arc<F>) -> Self {
        Self { services, infra }
    }

    /// Creates a PawsApp instance with the current services
    fn app(&self) -> PawsApp<A>
    where
        A: Services,
    {
        PawsApp::new(self.services.clone())
    }
}

impl PawsAPI<PawsServices<PawsRepo<PawsInfra>>, PawsRepo<PawsInfra>> {
    pub fn init(restricted: bool, cwd: PathBuf) -> Self {
        ...
        let app = Arc::new(PawsServices::new(repo.clone()));
        PawsAPI::new(app, repo)
    }
    ...
    async fn user_usage(&self) -> Result<Option<UserUsage>> {
        ...
        if let Some(api_key) = provider
            ...
            .and_then(|c| match &c.auth_details {
                paws_domain::AuthDetails::ApiKey(key) => Some(key.as_str()),
                _ => None,
            })
        {
