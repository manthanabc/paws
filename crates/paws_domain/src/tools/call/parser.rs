...
fn parse_tool_call(input: &str) -> IResult<&str, ToolCallParsed> {
    ...
    let (input, _) = tag("<paws_tool_call>").parse(input)?;
    let (input, _) = multispace0(input)?; // Handle whitespace after <paws_tool_call>

    // Match the tool name tags: <paws_tool_name>
    let (input, _) = tag("<").parse(input)?;
...
fn find_next_tool_call(input: &str) -> IResult<&str, &str> {
    ...
    let (remaining, _) = take_until("<paws_tool_call>").parse(input)?;
    Ok((remaining, ""))
}
...
mod tests {
    ...
    impl ToolCallBuilder {
        ...
        fn build_xml(&self) -> String {
            let mut xml = String::from("<paws_tool_call>");
            ...
            xml.push_str(&format!("</{}></paws_tool_call>", self.name));
            xml
        }
    ...
    fn test_parse_new_tool_call_format() {
        let input = r#"<paws_tool_call><search><path>/test/path</path><regex>test</regex></search></paws_tool_call>"#;
    ...
    fn test_parse_with_newlines() {
        let input = [
            "<paws_tool_call><foo><p1>",
            "abc",
            "</p1></foo></paws_tool_call>",
        ]
        .join("\n");
