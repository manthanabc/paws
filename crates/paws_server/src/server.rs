...
impl Server {
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let state =
            Arc::new(AppState { api: self.api.clone(), task_manager, task_store, broadcaster });
        ...
        let app = Router::new()
            ...
            .route(
                "/api/tasks/:id/stream/resumable",
                get(stream_task_events_resumable),
            )
            // Git
            ...
            // Configuration
            .route(
                "/api/config/default-provider",
                get(get_default_provider).post(set_default_provider),
            )
            .route(
            ...
            .route(
                "/api/config/active-agent",
                get(get_active_agent).post(set_active_agent),
            )
            // MCP Configuration
            .route(
                "/api/mcp/config",
                get(get_mcp_config).post(write_mcp_config),
            )
