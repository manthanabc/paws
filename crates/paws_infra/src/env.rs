...
use paws_app::EnvironmentInfra;
use paws_domain::{Environment, ModelId, ProviderId, RetryConfig, TlsBackend, TlsVersion};
use reqwest::Url;

#[derive(Clone)]
pub struct PawsEnvironmentInfra {
    restricted: bool,
    cwd: PathBuf,
}

impl PawsEnvironmentInfra {
    ...
    fn get(&self) -> Environment {
        ...
        Environment {
            ...
            workspace_server_url: parse_env::<String>("FORGE_WORKSPACE_SERVER_URL")
                .as_ref()
                .and_then(|url| Url::parse(url.as_str()).ok())
                .unwrap_or_else(|| Url::parse("https://api.pawscode.dev/").unwrap()),
            override_model,
            override_provider,
        }
    }
...
impl EnvironmentInfra for PawsEnvironmentInfra {
...
impl_from_env_str_via_from_str! {
    ...
    f32, f64,
    String,
    paws_domain::TlsBackend,
    paws_domain::TlsVersion,
}
...
fn resolve_http_config() -> paws_domain::HttpConfig {
    let mut config = paws_domain::HttpConfig::default();
...
mod tests {
    ...
    use paws_domain::{TlsBackend, TlsVersion};
    ...
    fn test_dot_env_loading() {
        ...
        PawsEnvironmentInfra::dot_env(&cwd);
    ...
    fn test_http_config_parsing() {
        ...
        let expected = paws_domain::HttpConfig::default();
    ...
    fn test_max_search_result_bytes() {
        ...
        // Test default value
        let paws_env = PawsEnvironmentInfra::new(false, PathBuf::from("/tmp"));
        let environment = paws_env.get_environment();
    ...
    fn test_auto_open_dump_env_var() {
        ...
        let infra = PawsEnvironmentInfra::new(false, cwd);
    ...
    fn test_max_image_size_env_var() {
        ...
        let infra = PawsEnvironmentInfra::new(false, cwd.path().to_path_buf());
