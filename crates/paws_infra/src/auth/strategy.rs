...
async fn poll_for_tokens(
    device_code: &paws_domain::DeviceCode,
    config: &OAuthConfig,
    timeout: Duration,
...
pub struct PawsAuthStrategyFactory {}

impl Default for PawsAuthStrategyFactory {
    fn default() -> Self {
        Self::new()
    }
}

impl PawsAuthStrategyFactory {
    pub fn new() -> Self {
        Self {}
    }
}

impl StrategyFactory for PawsAuthStrategyFactory {
    ...
    fn create_auth_strategy(
        ...
        match auth_method {
            paws_domain::AuthMethod::ApiKey => Ok(AnyAuthStrategy::ApiKey(ApiKeyStrategy::new(
                provider_id,
                required_params,
            ))),
            paws_domain::AuthMethod::OAuthCode(config) => {
...
mod tests {
    ...
    fn test_create_auth_strategy_api_key() {
        let factory = PawsAuthStrategyFactory::new();
