...
impl PawsDirectoryReaderService {
    ...
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
        ...
        if !PawsFS::exists(directory) || PawsFS::is_file(directory) {
            return Ok(vec![]);
        }

        // Read directory entries
        let mut dir = PawsFS::read_dir(directory).await?;
    ...
    async fn read_directory_files(
        ...
        let read_tasks = file_paths.into_iter().map(|path| {
            ...
            async move {
                match PawsFS::read_to_string(&path).await {
...
impl DirectoryReaderInfra for PawsDirectoryReaderService {
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
...
mod tests {
    ...
    async fn test_read_directory_files_with_filter() {
        ...
        let actual = PawsDirectoryReaderService
    ...
    async fn test_read_directory_files_without_filter() {
        ...
        let mut actual = PawsDirectoryReaderService
