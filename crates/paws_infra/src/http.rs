...
use anyhow::Context;
use bytes::Bytes;
use paws_app::HttpInfra;
use paws_domain::{Environment, TlsBackend, TlsVersion};
...
impl<F> PawsHttpInfra<F> {
    ...
    fn headers(&self, headers: Option<HeaderMap>) -> HeaderMap {
        ...
        headers.insert("User-Agent", HeaderValue::from_static("Paws"));
        headers.insert("X-Title", HeaderValue::from_static("paws"));
        headers.insert(
            "x-app-version",
        ...
        headers.insert(
            "HTTP-Referer",
            HeaderValue::from_static("https://pawscode.dev"),
        );
        headers.insert(
...
impl<F: paws_app::FileWriterInfra + 'static> PawsHttpInfra<F> {
...
impl<F: paws_app::FileWriterInfra + 'static> HttpInfra for PawsHttpInfra<F> {
...
mod tests {
    ...
    use paws_domain::{Environment, HttpConfig};
    ...
    async fn test_debug_requests_none_does_not_write() {
        ...
        let http = PawsHttpInfra::new(env, Arc::new(file_writer.clone()));
    ...
    async fn test_debug_requests_with_valid_path() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/paws-test/debug.json");
