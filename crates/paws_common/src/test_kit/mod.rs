//! Test utilities and helpers for Paws tests
//!
...
macro_rules! fixture {
    ($path:expr) => {
        $crate::test_kit::fixture(&format!("{}/{}", env!("CARGO_MANIFEST_DIR"), $path))
    };
}
...
macro_rules! json_fixture {
    ($path:expr) => {
        $crate::test_kit::json_fixture(&format!("{}/{}", env!("CARGO_MANIFEST_DIR"), $path))
    };
}
