...
use anyhow::Result;
use paws_app::ConversationService;
use paws_app::domain::{Conversation, ConversationId};
use paws_domain::ConversationRepository;

/// Service for managing conversations, including creation, retrieval, and
/// updates
#[derive(Clone)]
pub struct PawsConversationService<S> {
    conversation_repository: Arc<S>,
}

impl<S: ConversationRepository> PawsConversationService<S> {
    /// Creates a new PawsConversationService with the provided repository
...
impl<S: ConversationRepository> ConversationService for PawsConversationService<S> {
        ...
        T: Send,
    {
        let mut conversation = self
            ...
            .get_conversation(id)
            .await?
            .ok_or_else(|| paws_app::domain::Error::ConversationNotFound(*id))?;
