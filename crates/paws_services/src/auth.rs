...
use anyhow::bail;
use bytes::Bytes;
use paws_app::{AuthService, EnvironmentInfra, Error, HttpInfra, User, UserUsage};
use paws_domain::{AppConfigRepository, InitAuth, LoginInfo};
use reqwest::Url;
...
pub struct PawsAuthService<I> {
    infra: Arc<I>,
}

impl<I: HttpInfra + EnvironmentInfra + AppConfigRepository> PawsAuthService<I> {
    pub fn new(infra: Arc<I>) -> Self {
        Self { infra }
    }
    async fn init(&self) -> anyhow::Result<InitAuth> {
        let init_url = format!("{}{AUTH_ROUTE}", self.infra.get_environment().paws_api_url);
    ...
    async fn login(&self, auth: &InitAuth) -> anyhow::Result<LoginInfo> {
        let url = format!(
            "{}{AUTH_ROUTE}{}",
            self.infra.get_environment().paws_api_url,
            auth.session_id
        );
...
impl<I: HttpInfra + EnvironmentInfra + AppConfigRepository> AuthService for PawsAuthService<I> {
