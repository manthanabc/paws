...
impl<H: HttpClientService> OpenAIProvider<H> {
    ...
    fn get_headers(&self) -> Vec<(String, String)> {
        ...
        self.provider
            ...
            .for_each(|method| match method {
                paws_domain::AuthMethod::ApiKey => {}
                paws_domain::AuthMethod::OAuthDevice(oauth_config) => {
                ...
                paws_domain::AuthMethod::OAuthCode(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
    ...
    async fn inner_models(&self) -> Result<Vec<paws_app::domain::Model>> {
    ...
    fn inner_vertex_models(&self) -> Vec<paws_app::domain::Model> {
        lazy_static! {
            static ref VERTEX_MODELS: Vec<paws_app::domain::Model> = {
                let models =
...
impl<T: HttpClientService> OpenAIProvider<T> {
    ...
    pub async fn models(&self) -> Result<Vec<paws_app::domain::Model>> {
...
mod tests {
    ...
    use bytes::Bytes;
    use paws_app::HttpClientService;
    use paws_app::domain::{Provider, ProviderId, ProviderResponse};
    ...
    fn make_credential(provider_id: ProviderId, key: &str) -> Option<paws_domain::AuthCredential> {
        Some(paws_domain::AuthCredential {
            id: provider_id,
            auth_details: paws_domain::AuthDetails::ApiKey(paws_domain::ApiKey::from(
                key.to_string(),
            )),
    ...
    fn openai(key: &str) -> Provider<Url> {
        Provider {
            ...
            url_params: vec![],
            models: Some(paws_domain::ModelSource::Url(
