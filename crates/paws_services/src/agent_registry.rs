...
use dashmap::DashMap;
use paws_app::AgentRepository;
use paws_app::domain::AgentId;
use paws_domain::{Agent, AppConfigRepository, ProviderRepository};
...
/// access.
pub struct PawsAgentRegistryService<R> {
...
impl<R> PawsAgentRegistryService<R> {
...
impl<R: AgentRepository + AppConfigRepository + ProviderRepository> PawsAgentRegistryService<R> {
    ...
    async fn load_agents(&self) -> anyhow::Result<DashMap<String, Agent>> {
        ...
        let default_provider_id = app_config
            .provider
            .ok_or(paws_domain::Error::NoDefaultProvider)?;
...
impl<R: AgentRepository + AppConfigRepository + ProviderRepository> paws_app::AgentRegistry
    for PawsAgentRegistryService<R>
{
