...
impl<F> PawsDiscoveryService<F> {
    pub fn new(service: Arc<F>) -> Self {
        Self { service }
    }
}

impl<F: EnvironmentInfra + WalkerInfra> PawsDiscoveryService<F> {
...
mod tests {
    ...
    async fn test_list_current_directory_sorts_dirs_first() {
        ...
        let service = PawsDiscoveryService::new(Arc::new(fixture));
