...
#[derive(Clone)]
pub struct PawsTemplateService<F> {
    hb: Arc<RwLock<Handlebars<'static>>>,
    infra: Arc<F>,
}

impl<F: EnvironmentInfra + FileReaderInfra> PawsTemplateService<F> {
    pub fn new(infra: Arc<F>) -> Self {
        let hb = paws_app::TemplateEngine::handlebar_instance();
...
impl<F: EnvironmentInfra + FileReaderInfra> TemplateService for PawsTemplateService<F> {
...
mod tests {
    ...
    async fn test_render_simple_template() {
        ...
        let service = PawsTemplateService::new(Arc::new(MockCompositeService::new()));
        let data = json!({
            "name": "Paws",
            "version": "1.0",
        ...
        let expected = "App: Paws v1.0 - Features: templates, rendering, handlebars";
    ...
    async fn test_render_partial_system_info() {
        ...
        let actual = service
            .render_template(Template::new("{{> paws-partial-system-info.md }}"), &data)
            .await
            .unwrap();
