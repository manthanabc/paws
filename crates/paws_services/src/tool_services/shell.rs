...
use paws_app::{CommandInfra, EnvironmentInfra, ShellOutput, ShellService};
...
/// unrestricted access, advise users to run paws CLI with '-u' flag. Returns
/// complete output including stdout, stderr, and exit code for diagnostic
/// purposes.
pub struct PawsShell<I> {
    env: Environment,
    infra: Arc<I>,
}

impl<I: EnvironmentInfra> PawsShell<I> {
...
impl<I: CommandInfra + EnvironmentInfra> ShellService for PawsShell<I> {
    async fn execute(
        &self,
        command: String,
        cwd: PathBuf,
...
mod tests {
    ...
    use paws_app::domain::{CommandOutput, Environment};
    use paws_app::{CommandInfra, ShellService};
    ...
    async fn test_shell_service_forwards_env_vars() {
        let fixture = PawsShell::new(Arc::new(MockCommandInfra {
    ...
    async fn test_shell_service_forwards_no_env_vars() {
        let fixture = PawsShell::new(Arc::new(MockCommandInfra { expected_env_vars: None }));
