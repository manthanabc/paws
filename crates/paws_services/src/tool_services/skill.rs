...
use paws_app::SkillFetchService;
use paws_domain::Skill;
...
pub struct PawsSkillFetch<R> {
    repository: Arc<R>,
    cache: OnceCell<Vec<Skill>>,
}

impl<R> PawsSkillFetch<R> {
...
impl<R: paws_domain::SkillRepository> SkillFetchService for PawsSkillFetch<R> {
...
impl<R: paws_domain::SkillRepository> PawsSkillFetch<R> {
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl paws_domain::SkillRepository for MockSkillRepository {
    ...
    async fn test_fetch_skill_found() {
        ...
        let fetch_service = PawsSkillFetch::new(Arc::new(repo));
