...
use anyhow::Context;
use paws_app::domain::Image;
use paws_app::{EnvironmentInfra, FileInfoInfra, ImageReadService};
use strum_macros::{Display, EnumString};

use crate::utils::assert_absolute_path;

pub struct PawsImageRead<F>(Arc<F>);
...
impl<F> PawsImageRead<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self(infra)
    }
}
#[async_trait::async_trait]
impl<F: FileInfoInfra + EnvironmentInfra + paws_app::FileReaderInfra> ImageReadService
    for PawsImageRead<F>
{
