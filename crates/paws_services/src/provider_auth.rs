...
use paws_app::{AuthStrategy, ProviderAuthService, StrategyFactory};
use paws_domain::{
    AuthContextRequest, AuthContextResponse, AuthMethod, Provider, ProviderId, ProviderRepository,
};

/// Paws Provider Authentication Service
#[derive(Clone)]
pub struct PawsProviderAuthService<I> {
    infra: Arc<I>,
}

impl<I> PawsProviderAuthService<I> {
...
impl<I> ProviderAuthService for PawsProviderAuthService<I>
where
    ...
    async fn init_provider_auth(
        ...
        let required_params = if matches!(auth_method, AuthMethod::ApiKey) {
            ...
            let provider = providers
                ...
                .ok_or_else(|| paws_domain::Error::provider_not_available(provider_id.clone()))?;
                        ...
                        AuthMethod::OAuthDevice(_) | AuthMethod::OAuthCode(_) => {
                            ...
                            let existing_credential =
                                self.infra.get_credential(&provider.id).await?.ok_or_else(
                                    || paws_domain::Error::ProviderNotAvailable {
