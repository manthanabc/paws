...
use crate::tracker::can_track::can_track;
...
pub fn init_tracing(log_path: PathBuf, tracker: Tracker) -> anyhow::Result<Guard> {
    ...
    // Create a filter that only allows logs from paws_ modules
    let filter = filter::filter_fn(|metadata| metadata.target().starts_with("paws_"));
...
fn prepare_writer(
    ...
    let ((non_blocking, guard), env) = if can_track() {
        ...
        (
            ...
            tracing_subscriber::EnvFilter::new("paws=info"),
        )
    } else {
        let append = tracing_appender::rolling::daily(log_path, "paws.log");
        (
            tracing_appender::non_blocking(append),
            tracing_subscriber::EnvFilter::new("paws=debug"),
        )
    };
...
impl std::io::Write for PostHogWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        let event_kind = crate::tracker::EventKind::Trace(buf.to_vec());
