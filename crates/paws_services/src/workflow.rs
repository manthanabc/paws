...
use anyhow::Context;
use paws_app::domain::Workflow;
use paws_app::{FileReaderInfra, FileWriterInfra, WorkflowService};

/// A workflow loader to load the workflow from the given path.
/// It also resolves the internal paths specified in the workflow.
pub struct PawsWorkflowService<F> {
    infra: Arc<F>,
}

impl<F> PawsWorkflowService<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self { infra }
    }
}

impl<F: FileWriterInfra + FileReaderInfra> PawsWorkflowService<F> {
    /// Find a paws.yaml config file by traversing parent directories.
    ...
    pub async fn resolve_path(&self, path: Option<PathBuf>) -> PathBuf {
        ...
        if path.exists() || path.to_string_lossy() != "paws.yaml" {
    ...
    /// If the path is just "paws.yaml", searches for it in parent directories.
...
impl<F: FileWriterInfra + FileReaderInfra> WorkflowService for PawsWorkflowService<F> {
    ...
    async fn read_workflow(&self, path: Option<&Path>) -> anyhow::Result<Workflow> {
        let path_to_use = path.unwrap_or_else(|| Path::new("paws.yaml"));
...
mod tests {
    ...
    fn test_find_config_file_behavior() {
        ...
        let config_path = temp_dir.path().join("paws.yaml");
        fs::write(&config_path, "test content").unwrap();

        let result = find_config_file_logic(Path::new("paws.yaml"), &config_path);
        assert_eq!(result, config_path);

        // Test 2: Return original path for non-paws.yaml files
        ...
        let parent_config = parent_dir.join("paws.yaml");
        fs::write(&parent_config, "parent config").unwrap();

        let result = find_config_file_logic(Path::new("paws.yaml"), &parent_config);
    ...
    fn test_explicit_path_not_searched() {
        ...
        // Create paws.yaml in the parent
        fs::write(parent_dir.join("paws.yaml"), "# Test").unwrap();

        // Simulate search with a non-paws.yaml path
        let custom_path = PathBuf::from("custom-config.yaml");
        let parent_config = parent_dir.join("paws.yaml");
