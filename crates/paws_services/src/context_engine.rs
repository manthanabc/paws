...
struct SyncPlan {
    ...
    files_to_upload: Vec<paws_domain::FileRead>,
...
impl SyncPlan {
    ...
    fn new(local_files: Vec<IndexedFile>, remote_files: Vec<FileHash>) -> Self {
        ...
        let files_to_upload: Vec<_> = local_files
            ...
            .map(|f| paws_domain::FileRead::new(f.path, f.content))
            .collect();
    ...
    async fn execute<'a>(
        ...
        upload: impl Fn(Vec<paws_domain::FileRead>) -> BoxFuture<'a, Result<()>>,
...
pub struct PawsContextEngineService<F> {
    infra: Arc<F>,
}

impl<F> Clone for PawsContextEngineService<F> {
    fn clone(&self) -> Self {
        Self { infra: Arc::clone(&self.infra) }
    }
}

impl<F> PawsContextEngineService<F> {
    ...
    async fn delete(
        ...
        user_id: &UserId,
        workspace_id: &WorkspaceId,
        token: &paws_domain::ApiKey,
        paths: Vec<String>,
    ) -> Result<()>
    where
        F: ContextEngineRepository,
    {
        let deletion = paws_domain::CodeBase::new(user_id.clone(), workspace_id.clone(), paths);
        self.infra
    ...
    async fn upload(
        ...
        workspace_id: &WorkspaceId,
        token: &paws_domain::ApiKey,
        files: Vec<paws_domain::FileRead>,
    ) -> Result<()>
    where
        F: ContextEngineRepository,
    {
        let upload = paws_domain::CodeBase::new(user_id.clone(), workspace_id.clone(), files);
        self.infra
    ...
    fn extract_workspace_auth(
    ...
    ) -> Result<(paws_domain::ApiKey, UserId)> {
        use paws_domain::AuthDetails;
        ...
        match &credential.auth_details {
            ...
            _ => anyhow::bail!("PawsServices credential must be an API key"),
        }
    }

    /// Convert WorkspaceAuth to AuthCredential for storage
    fn workspace_auth_to_credential(auth: &paws_domain::WorkspaceAuth) -> AuthCredential {
        use std::collections::HashMap;
...
impl<
    F: WorkspaceRepository
        ...
        + FileReaderInfra
        + 'static,
> ContextEngineService for PawsContextEngineService<F>
{
    ...
    async fn query_codebase(
        ...
        let workspace = self
            ...
            .ok_or(paws_domain::Error::WorkspaceNotFound)?;
        ...
        let credential = self
            ...
            .get_credential(&ProviderId::FORGE_SERVICES)
            .await?
            .ok_or(paws_domain::Error::AuthTokenNotFound)?;

        let (token, _) = Self::extract_workspace_auth(&credential)?;

        // Step 4: Search the codebase
        let search_query = paws_domain::CodeBase::new(
    ...
    async fn list_codebase(&self) -> Result<Vec<paws_domain::WorkspaceInfo>> {
        // Get auth token
        let credential = self
            .infra
            .get_credential(&ProviderId::FORGE_SERVICES)
    ...
    async fn get_workspace_info(&self, path: PathBuf) -> Result<Option<paws_domain::WorkspaceInfo>>
    where
...
// Additional authentication methods for PawsIndexingService
impl<F> PawsContextEngineService<F>
where
...
mod tests {
    ...
    fn search_result() -> Node {
        Node {
            node_id: "n1".into(),
            node: paws_domain::NodeData::FileChunk(paws_domain::FileChunk {
    ...
    impl ProviderRepository for MockInfra {
        async fn get_all_providers(&self) -> Result<Vec<paws_domain::AnyProvider>> {
            Ok(vec![])
        }

        async fn get_provider(&self, _id: ProviderId) -> Result<paws_domain::Provider<url::Url>> {
    ...
    async fn test_query_returns_results() {
        ...
        let service = PawsContextEngineService::new(Arc::new(mock));

        let params = paws_domain::SearchParams::new("test", "fest").limit(10usize);
        let actual = service
    ...
    async fn test_query_error_when_not_found() {
        let service = PawsContextEngineService::new(Arc::new(MockInfra::default()));
    ...
    async fn test_orphaned_files_deleted() {
        ...
        let service = PawsContextEngineService::new(Arc::new(mock.clone()));
    ...
    async fn test_sync_codebase_emits_progress_events() {
        ...
        let has_completion = events
            .iter()
            .any(|e| matches!(e, paws_domain::SyncProgress::Completed { .. }));
    ...
    async fn test_sync_plan_execute_batches_and_tracks_progress() {
        ...
        let plan = SyncPlan {
            ...
            files_to_upload: vec![
                paws_domain::FileRead::new("d.rs".into(), "content_d".into()),
                paws_domain::FileRead::new("e.rs".into(), "content_e".into()),
            ],
    ...
    async fn test_sync_plan_execute_with_modified_files() {
        ...
        let plan = SyncPlan {
            ...
            files_to_upload: vec![
                paws_domain::FileRead::new("a.rs".into(), "new_content".into()),
                paws_domain::FileRead::new("c.rs".into(), "content_c".into()),
            ],
