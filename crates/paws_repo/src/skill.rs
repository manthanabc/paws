...
/// 2. Global custom skills (from ~/paws/skills/ directory)
/// 3. Project-local skills (from .paws/skills/ directory in current working
///    directory)
///
/// ## Skill Precedence
...
/// - **Global skills**: `~/paws/skills/<skill-name>/SKILL.md`
/// - **CWD skills**: `./.paws/skills/<skill-name>/SKILL.md` (relative to
///   current working directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct PawsSkillRepository<I> {
    infra: Arc<I>,
}

impl<I> PawsSkillRepository<I> {
    ...
    fn load_builtin_skills(&self) -> Vec<Skill> {
        let builtin_skills = vec![
            (
                "paws://skills/create-skill/SKILL.md",
                include_str!("skills/create-skill/SKILL.md"),
            ),
            (
                "paws://skills/execute-plan/SKILL.md",
...
impl<I: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra> PawsSkillRepository<I> {
    ...
    fn render_skill(&self, skill: Skill, env: &paws_domain::Environment) -> anyhow::Result<Skill> {
        ...
        let rendered_command = TemplateEngine::default()
            .render_template(paws_domain::Template::new(&skill.command), &skill_context)?;
...
mod tests {
    ...
    fn fixture_skill_repo() -> (PawsSkillRepository<PawsInfra>, std::path::PathBuf) {
        let skill_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/fixtures/skills_with_resources");
        let infra = Arc::new(PawsInfra::new(false, std::env::current_dir().unwrap()));
        let repo = PawsSkillRepository::new(infra);
        (repo, skill_dir)
    }
    ...
    fn test_load_builtin_skills() {
        // Fixture
        let repo = PawsSkillRepository { infra: Arc::new(()) };
        ...
        assert_eq!(
            create_skill.path,
            Some(std::path::Path::new("paws://skills/create-skill/SKILL.md").to_path_buf())
        );
        assert_eq!(
        ...
        assert_eq!(
            execute_plan.path,
            Some(std::path::Path::new("paws://skills/execute-plan/SKILL.md").to_path_buf())
        );
        assert!(execute_plan
            .description
