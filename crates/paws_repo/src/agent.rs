...
/// 2. Global custom agents (from ~/.paws/agents/ directory)
/// 3. Project-local agents (from .paws/agents/ directory in current working
///    directory)
///
/// ## Agent Precedence
...
/// - **Global agents**: `{HOME}/.paws/agents/*.md`
/// - **CWD agents**: `./.paws/agents/*.md` (relative to current working
///   directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct PawsAgentRepository<I> {
    infra: Arc<I>,
}

impl<I> PawsAgentRepository<I> {
...
impl<I: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra> AgentRepository
    for PawsAgentRepository<I>
{
    /// Load all agent definitions from all available sources with conflict
    /// resolution.
    async fn get_agents(&self) -> anyhow::Result<Vec<paws_app::domain::AgentDefinition>> {
        self.load_agents().await
    }
}

impl<I: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra> PawsAgentRepository<I> {
    ...
    async fn init_default(&self) -> anyhow::Result<Vec<AgentDefinition>> {
        parse_agent_iter(
            [
                ("paws", include_str!("agents/paws.md")),
