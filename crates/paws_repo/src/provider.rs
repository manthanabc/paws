...
enum Models {
    ...
    Hardcoded(Vec<paws_app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<paws_domain::AuthMethod>,
}
    ...
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => {
                paws_domain::ModelSource::Url(paws_domain::Template::new(model_url_template))
            }
            Models::Hardcoded(model_list) => {
                paws_domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: paws_domain::Template::new(&config.url),
...
pub struct PawsProviderRepository<F> {
    infra: Arc<F>,
    handlebars: &'static Handlebars<'static>,
}

impl<F> PawsProviderRepository<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self { infra, handlebars: get_handlebars() }
    }
}

impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra> PawsProviderRepository<F> {
    ...
    async fn get_providers(&self) -> Vec<AnyProvider> {
        ...
        for config in configs {
            // Skip Paws provider as it's handled specially
    ...
    pub async fn migrate_env_to_file(&self) -> anyhow::Result<Option<MigrationResult>> {
        ...
        for config in configs {
            // Skip Paws provider and ContextEngine providers - they're not configurable
    ...
    async fn provider_from_id(&self, id: ProviderId) -> anyhow::Result<Provider<Url>> {
        ...
        if id == ProviderId::FORGE {
            // Paws provider isn't typically configured via env vars in the registry
...
#[cfg(test)]
mod tests {
    use paws_app::domain::ProviderResponse;
...
mod env_tests {
    ...
    async fn test_migration_from_env_to_file() {
        ...
        let registry = PawsProviderRepository::new(infra.clone());
    ...
    #[tokio::test]
    async fn test_migration_should_not_create_paws_services_credential() {
        let mut env_vars = HashMap::new();
    ...
    async fn test_create_azure_provider_with_handlebars_urls() {
        ...
        let infra = Arc::new(MockInfra::new(env_vars));
        let registry = PawsProviderRepository::new(infra);
        ...
        assert_eq!(
            provider
                ...
                .and_then(|c| match &c.auth_details {
                    paws_domain::AuthDetails::ApiKey(key) => Some(key.to_string()),
                    _ => None,
                }),
        ...
        match &provider.models.as_ref().unwrap() {
            ...
            paws_domain::ModelSource::Hardcoded(_) => panic!("Expected Models::Url variant"),
        }
    }
