...
//! `paws_domain` counterparts for compile-time safety while keeping the
...
impl From<&paws_domain::ModelId> for ModelIdRecord {
    fn from(id: &paws_domain::ModelId) -> Self {
        Self(id.to_string())
    }
}

impl From<ModelIdRecord> for paws_domain::ModelId {
    fn from(record: ModelIdRecord) -> Self {
        paws_domain::ModelId::from(record.0)
    }
}
...
impl From<&paws_domain::Image> for ImageRecord {
    fn from(image: &paws_domain::Image) -> Self {
        Self {
...
impl From<ImageRecord> for paws_domain::Image {
    fn from(record: ImageRecord) -> Self {
        paws_domain::Image::new_base64(
            record
                .url
...
impl From<&paws_domain::ToolCallId> for ToolCallIdRecord {
    fn from(id: &paws_domain::ToolCallId) -> Self {
        Self(id.as_str().to_string())
    }
}

impl From<ToolCallIdRecord> for paws_domain::ToolCallId {
    fn from(record: ToolCallIdRecord) -> Self {
        paws_domain::ToolCallId::new(record.0)
    }
}
...
impl From<&paws_domain::ToolCallArguments> for ToolCallArgumentsRecord {
    fn from(args: &paws_domain::ToolCallArguments) -> Self {
        // Serialize to JSON to capture both Parsed and Unparsed variants
        Self(serde_json::to_value(args).unwrap_or_default())
    }
}

impl From<ToolCallArgumentsRecord> for paws_domain::ToolCallArguments {
...
impl From<&paws_domain::ToolName> for ToolNameRecord {
    fn from(name: &paws_domain::ToolName) -> Self {
        Self(name.to_string())
    }
}

impl From<ToolNameRecord> for paws_domain::ToolName {
    fn from(record: ToolNameRecord) -> Self {
        paws_domain::ToolName::new(record.0)
    }
}
...
impl From<&paws_domain::ToolCallFull> for ToolCallFullRecord {
    fn from(call: &paws_domain::ToolCallFull) -> Self {
        Self {
...
impl From<ToolCallFullRecord> for paws_domain::ToolCallFull {
    fn from(record: ToolCallFullRecord) -> Self {
        paws_domain::ToolCallFull {
...
impl From<&paws_domain::ReasoningFull> for ReasoningFullRecord {
    fn from(reasoning: &paws_domain::ReasoningFull) -> Self {
        Self {
...
impl From<ReasoningFullRecord> for paws_domain::ReasoningFull {
    fn from(record: ReasoningFullRecord) -> Self {
        paws_domain::ReasoningFull {
            text: record.text,
...
impl From<&paws_domain::TokenCount> for TokenCountRecord {
    fn from(count: &paws_domain::TokenCount) -> Self {
        match count {
            paws_domain::TokenCount::Actual(n) => Self::Actual(*n),
            paws_domain::TokenCount::Approx(n) => Self::Approx(*n),
        }
    }
}

impl From<TokenCountRecord> for paws_domain::TokenCount {
...
impl From<&paws_domain::Usage> for UsageRecord {
    fn from(usage: &paws_domain::Usage) -> Self {
        Self {
...
impl From<UsageRecord> for paws_domain::Usage {
...
impl From<&paws_domain::EventValue> for EventValueRecord {
    fn from(event: &paws_domain::EventValue) -> Self {
        Self(serde_json::to_value(event).unwrap_or_default())
    }
}

impl TryFrom<EventValueRecord> for paws_domain::EventValue {
...
impl From<&paws_domain::Role> for RoleRecord {
    fn from(role: &paws_domain::Role) -> Self {
        match role {
            paws_domain::Role::System => Self::System,
            paws_domain::Role::User => Self::User,
            paws_domain::Role::Assistant => Self::Assistant,
        }
    }
}

impl From<RoleRecord> for paws_domain::Role {
...
impl From<&paws_domain::TextMessage> for TextMessageRecord {
    fn from(msg: &paws_domain::TextMessage) -> Self {
        Self {
...
impl TryFrom<TextMessageRecord> for paws_domain::TextMessage {
    type Error = anyhow::Error;

    fn try_from(record: TextMessageRecord) -> anyhow::Result<Self> {
        Ok(paws_domain::TextMessage {
...
impl From<&paws_domain::ToolValue> for ToolValueRecord {
    fn from(value: &paws_domain::ToolValue) -> Self {
        match value {
            paws_domain::ToolValue::Text(text) => Self::Text(text.clone()),
            paws_domain::ToolValue::AI { value, conversation_id } => Self::AI {
                value: value.clone(),
                conversation_id: conversation_id.into_string(),
            },
            paws_domain::ToolValue::Image(img) => Self::Image(ImageRecord::from(img)),
            paws_domain::ToolValue::Empty => Self::Empty,
        }
    }
}

impl TryFrom<ToolValueRecord> for paws_domain::ToolValue {
    type Error = anyhow::Error;
...
impl From<&paws_domain::ToolOutput> for ToolOutputRecord {
    fn from(output: &paws_domain::ToolOutput) -> Self {
        Self {
...
impl TryFrom<ToolOutputRecord> for paws_domain::ToolOutput {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ToolOutputRecord) -> anyhow::Result<Self> {
        ...
        Ok(paws_domain::ToolOutput { is_error: record.is_error, values: values? })
    }
}
...
impl From<&paws_domain::ToolResult> for ToolResultRecord {
    fn from(result: &paws_domain::ToolResult) -> Self {
        Self {
...
impl TryFrom<ToolResultRecord> for paws_domain::ToolResult {
    type Error = anyhow::Error;

    fn try_from(record: ToolResultRecord) -> anyhow::Result<Self> {
        Ok(paws_domain::ToolResult {
            name: record.name.into(),
            call_id: record.call_id.map(Into::into),
...
impl From<&paws_domain::ContextMessage> for ContextMessageValueRecord {
...
impl TryFrom<ContextMessageValueRecord> for paws_domain::ContextMessage {
    type Error = anyhow::Error;
...
impl From<&paws_domain::MessageEntry> for ContextMessageRecord {
    fn from(msg: &paws_domain::MessageEntry) -> Self {
        Self {
...
impl TryFrom<ContextMessageRecord> for paws_domain::MessageEntry {
    type Error = anyhow::Error;

    fn try_from(record: ContextMessageRecord) -> anyhow::Result<Self> {
        Ok(paws_domain::MessageEntry {
...
impl From<&paws_domain::ToolDefinition> for ToolDefinitionRecord {
    fn from(def: &paws_domain::ToolDefinition) -> Self {
        Self {
...
impl TryFrom<ToolDefinitionRecord> for paws_domain::ToolDefinition {
    type Error = anyhow::Error;

    fn try_from(record: ToolDefinitionRecord) -> anyhow::Result<Self> {
        Ok(paws_domain::ToolDefinition {
            name: record.name.into(),
...
impl From<&paws_domain::ToolChoice> for ToolChoiceRecord {
    fn from(choice: &paws_domain::ToolChoice) -> Self {
        match choice {
            paws_domain::ToolChoice::None => Self::None,
            paws_domain::ToolChoice::Auto => Self::Auto,
            paws_domain::ToolChoice::Required => Self::Required,
            paws_domain::ToolChoice::Call(name) => Self::Call(ToolNameRecord::from(name)),
        }
    }
}

impl From<ToolChoiceRecord> for paws_domain::ToolChoice {
...
impl From<&paws_domain::Effort> for EffortRecord {
    fn from(effort: &paws_domain::Effort) -> Self {
        match effort {
            paws_domain::Effort::High => Self::High,
            paws_domain::Effort::Medium => Self::Medium,
            paws_domain::Effort::Low => Self::Low,
        }
    }
}

impl From<EffortRecord> for paws_domain::Effort {
...
impl From<&paws_domain::ReasoningConfig> for ReasoningConfigRecord {
    fn from(config: &paws_domain::ReasoningConfig) -> Self {
        Self {
...
impl From<ReasoningConfigRecord> for paws_domain::ReasoningConfig {
    fn from(record: ReasoningConfigRecord) -> Self {
        paws_domain::ReasoningConfig {
...
impl TryFrom<ContextRecord> for Context {
    ...
    fn try_from(record: ContextRecord) -> anyhow::Result<Self> {
        ...
        Ok(Context {
            ...
            temperature: record
                .temperature
                .map(paws_domain::Temperature::new_unchecked),
            top_p: record.top_p.map(paws_domain::TopP::new_unchecked),
            top_k: record.top_k.map(paws_domain::TopK::new_unchecked),
...
pub(super) struct FileChangeMetricsRecord {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool: Option<paws_domain::ToolKind>,
}

impl From<&paws_domain::FileOperation> for FileChangeMetricsRecord {
    fn from(metrics: &paws_domain::FileOperation) -> Self {
        Self {
...
impl From<FileChangeMetricsRecord> for paws_domain::FileOperation {
    fn from(record: FileChangeMetricsRecord) -> Self {
        // Use Write as default tool for old records without tool field
        let tool = record.tool.unwrap_or(paws_domain::ToolKind::Write);
        Self::new(tool)
...
impl From<&paws_domain::Metrics> for MetricsRecord {
    fn from(metrics: &paws_domain::Metrics) -> Self {
        Self {
...
impl From<MetricsRecord> for paws_domain::Metrics {
...
impl ConversationRecord {
    ...
    pub fn new(
        conversation: paws_domain::Conversation,
        workspace_id: paws_domain::WorkspaceHash,
    ) -> Self {
...
impl TryFrom<ConversationRecord> for paws_domain::Conversation {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ConversationRecord) -> anyhow::Result<Self> {
        ...
        let metrics = record
            ...
            .map(paws_domain::Metrics::from)
            .unwrap_or_else(|| {
                paws_domain::Metrics::default().started_at(record.created_at.and_utc())
            });

        Ok(paws_domain::Conversation::new(id)
            .context(context)
            .title(record.title)
            .metrics(metrics)
            .metadata(
                paws_domain::MetaData::new(record.created_at.and_utc())
