...
#[allow(dead_code)]
mod proto_generated {
    tonic::include_proto!("paws.v1");
}

use paws_service_client::PawsServiceClient;
use proto_generated::*;
...
impl TryFrom<FileRefNode> for paws_domain::FileHash {
    type Error = anyhow::Error;

    fn try_from(file_ref_node: FileRefNode) -> Result<Self> {
        let data = file_ref_node.data.context("Missing data in FileRefNode")?;
        Ok(paws_domain::FileHash { path: data.path, hash: data.file_hash })
    }
}

/// gRPC implementation of CodebaseRepository
pub struct PawsContextEngineRepository<I> {
    infra: Arc<I>,
}

impl<I> PawsContextEngineRepository<I> {
...
#[async_trait]
impl<I: GrpcInfra> ContextEngineRepository for PawsContextEngineRepository<I> {
    async fn authenticate(&self) -> Result<WorkspaceAuth> {
        let channel = self.infra.channel();
        let mut client = PawsServiceClient::new(channel);
    ...
    async fn search(
        &self,
        search_query: &paws_domain::CodeSearchQuery<'_>,
        auth_token: &paws_domain::ApiKey,
    ) -> Result<Vec<Node>> {
            ...
            .filter_map(|query_item| {
                ...
                let code_node = match node_data.kind? {
                    node_data::Kind::FileChunk(chunk) => {
                        paws_domain::NodeData::FileChunk(paws_domain::FileChunk {
                    ...
                    node_data::Kind::File(file) => {
                        paws_domain::NodeData::File(paws_domain::FileNode {
                            file_path: file.path,
                    ...
                    node_data::Kind::FileRef(file_ref) => {
                        paws_domain::NodeData::FileRef(paws_domain::FileRef {
                            file_path: file_ref.path,
                            file_hash: file_ref.file_hash,
                        })
                    }
                    node_data::Kind::Note(note) => {
                        paws_domain::NodeData::Note(paws_domain::Note { content: note.content })
                    }
                    node_data::Kind::Task(task) => {
                        paws_domain::NodeData::Task(paws_domain::Task { task: task.task })
                    }
                };
    ...
    async fn list_workspace_files(
        ...
        auth_token: &paws_domain::ApiKey,
    ) -> Result<Vec<paws_domain::FileHash>> {
