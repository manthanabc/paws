...
use diesel::prelude::*;
use paws_domain::{UserId, Workspace, WorkspaceId, WorkspaceRepository};

use crate::database::schema::workspace;
use crate::database::DatabasePool;

/// Repository implementation for workspace persistence in local database
pub struct PawsWorkspaceRepository {
    pool: Arc<DatabasePool>,
}

impl PawsWorkspaceRepository {
...
impl WorkspaceRepository for PawsWorkspaceRepository {
    async fn upsert(
        &self,
...
mod tests {
    ...
    use paws_domain::UserId;
    use pretty_assertions::assert_eq;

    use super::*;
    use crate::database::DatabasePool;

    fn repo_impl() -> PawsWorkspaceRepository {
        let pool = Arc::new(DatabasePool::in_memory().unwrap());
        PawsWorkspaceRepository::new(pool)
    }
