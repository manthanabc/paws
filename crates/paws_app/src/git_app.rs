        ...
        + ProviderAuthService,
{
    ...
    async fn generate_message_from_diff(&self, ctx: DiffContext) -> Result<CommitMessageDetails> {
        ...
        let (rendered_prompt, provider, model) = tokio::try_join!(
            self.services
                .render_template(Template::new("{{> paws-commit-message-prompt.md }}"), &()),
        ...
        let context = paws_domain::Context::default()
        ...
        let commit_message = paws_domain::extract_tag_content(&message.content, "commit_message")
