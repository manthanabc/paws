...
impl FormatContent for ToolOperation {
    fn to_content(&self, env: &Environment) -> Option<ChatResponseContent> {
        match self {
            ...
            | ToolOperation::ImageRead { input: _, output: _ }
...
mod tests {
    ...
    fn test_fs_remove() {
        let fixture = ToolOperation::FsRemove {
            input: paws_domain::FSRemove { path: "/home/user/project/file.txt".to_string() },
    ...
    fn test_fs_undo() {
        let fixture = ToolOperation::FsUndo {
            input: paws_domain::FSUndo { path: "/home/user/project/test.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_plan_create() {
        ...
        if let Some(ChatResponseContent::Title(title)) = actual {
            ...
            assert_eq!(title.category, paws_domain::Category::Debug);
