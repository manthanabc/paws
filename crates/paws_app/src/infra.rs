...
/// Environment exposes:
/// - Global agent directory via `agent_path()` (typically ~/.paws/agents)
...
/// This uses the EnvironmentService trait from paws_domain
...
pub trait AuthStrategy: Send + Sync {
    ...
    async fn init(&self) -> anyhow::Result<paws_domain::AuthContextRequest>;

    /// Complete authentication flow
    async fn complete(
        &self,
        context_response: paws_domain::AuthContextResponse,
    ) -> anyhow::Result<paws_domain::AuthCredential>;
