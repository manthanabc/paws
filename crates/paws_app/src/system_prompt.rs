    ...
    pub async fn add_system_message(
        ...
        let context = if let Some(system_prompt) = &agent.system_prompt {
            ...
            let ctx = SystemContext {
                ...
                skills,
                tool_names: paws_domain::ToolNames::new(),
            };
            ...
            let non_static_block = TemplateEngine::default()
                .render_template(Template::new("{{> paws-custom-agent-template.md }}"), &ctx)?;
...
mod tests {
    ...
    use fake::Fake;
    use paws_domain::{Agent, Environment};
    ...
    impl SkillFetchService for MockSkillFetchService {
        async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<paws_domain::Skill> {
            Ok(
                paws_domain::Skill::new("test_skill", "Test skill", "Test skill description")
                    .path("/skills/test.md"),
            )
        }
    ...
    async fn test_system_prompt_adds_context() {
        ...
        let conversation = paws_domain::Conversation::generate();
