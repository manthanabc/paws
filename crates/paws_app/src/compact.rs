    ...
    fn compress_single_sequence(
        ...
        if let Some(reasoning) = reasoning_details
            && let Some(ContextMessage::Text(msg)) = context
                .messages
                .iter_mut()
                .find(|msg| msg.has_role(paws_domain::Role::Assistant))
                .map(|msg| &mut **msg)
            && msg
                .reasoning_details
                .as_ref()
...
mod tests {
    ...
    fn render_template(data: &serde_json::Value) -> String {
        TemplateEngine::default()
            .render("paws-partial-summary-frame.md", data)
            .unwrap()
    }

    #[test]
    fn test_template_engine_renders_summary_frame() {
        use paws_domain::{ContextSummary, Role, SummaryBlock, SummaryMessage, SummaryToolCall};
            ...
            SummaryBlock::new(
                ...
                vec![
                    SummaryToolCall::codebase_search(
                        vec![paws_domain::SearchQuery::new(
    ...
    async fn test_render_summary_frame_snapshot() {
        ...
        let environment = test_environment().cwd(PathBuf::from(
            "/Users/tushar/Documents/Projects/code-paws-workspace/code-paws",
        ));
        let compactor = Compactor::new(Compact::new(), environment);
    ...
    fn test_compaction_preserves_usage_information() {
        use paws_domain::{TokenCount, Usage};
